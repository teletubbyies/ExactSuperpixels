...
image = imread('./rgb_fg.jpg')
# Usage
# image: Your input image (shape: height x width x channels)
# n_desired: The exact number of superpixels you want
result = exact_num_superpixels(image, n_desired=256)

# Visualize
plt.subplot(1, 2, 1)
plt.imshow(image)
plt.title("Object without background")

plt.subplot(1, 2, 2)
plt.imshow(mark_boundaries(image, result))
